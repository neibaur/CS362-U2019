...
int playBaron(int currentPlayer, int choice1, struct gameState *state)
{
  ...
  if(choice1 > 0) {//Boolean true or going to discard an estate
    ...
    while(card_not_discarded) {
      if(state->hand[currentPlayer][p] == estate) {//Found an estate card!
      ...
      } else if(p > 4) { // p>4 looks like a bug compared to p > state->handCount[currentPlayer]
        if(DEBUG) {
        ...
        if(supplyCount(estate, state) > 0) {
            //missing code here for gainCard
...
int playMinion(int currentPlayer, int choice1, int choice2, int handPos, struct gameState *state)
{
  ...
  if(choice1) {//+2 coins
    ...
    for(i = 0; i < state->numPlayers; i++) {
      //discard hand
        // bug found - missing conditional if (i!=currentPlayer)
        // bug found - extra print statements going to console
        // bug found - missing conditional (if(state->handCount[i] > 4))
